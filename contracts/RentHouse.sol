...
contract RentHouse {
     ...
     */
    function makePropertyAsInActive(uint256 property_id) public {
        Property storage property = properties[property_id];
    ...
    /*
     * @notice Method for withdrawing funds of owner
     * @params property_id: Property ID
     * @params booking_id: Booking ID
     * @dev Only owner of that property can withdraw their money
     */
    function withdraw(uint256 property_id, uint256 booking_id) public payable {
        if (properties[property_id].owner != msg.sender) {
            revert RentHouse__NotOwner();
        }
        (bool _sent, ) = msg.sender.call{
